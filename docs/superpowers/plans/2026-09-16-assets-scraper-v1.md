...
- [ ] **Step 2: Run, see failures** **Step 3: Implement** following spec 11.2 (`verifyAssetParams`, `safeFetch` with `Referer` = origin of the asset URL's page is unknown here, so send `Referer: https://<asset host>/`, magic-byte sniffing for PNG, JPEG, GIF, WebP, AVIF, ICO, WOFF, WOFF2, TTF, OTF, SVG text, `wawoff2.decompress` for `fmt=ttf` after `parseFontBinary` and `isConvertibleFont`, `takeProxyBytes` with the upstream `content-length` or the streamed count). Route file: `export const runtime = "nodejs"; export const GET = (request: Request) => handleAssetRequest(request);`. Health route returns `{ ok: true, version: process.env.VERCEL_GIT_COMMIT_SHA ?? "dev", disabled: process.env.SCAN_DISABLED === "1", accessCode: Boolean(process.env.ACCESS_CODE) }` with `cache-control: no-store`. **Step 4: Run** (PASS) **Step 5: Commit** `feat(api): add signed asset proxy and health routes`
