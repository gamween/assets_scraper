...
function Input({ className, type = "text", ...props }: React.ComponentProps<"input">) {
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        "h-8 w-full min-w-0 rounded-md border border-border bg-surface px-2.5 text-body text-text transition-[border-color,box-shadow] duration-100 outline-none placeholder:text-text-3/80 hover:border-border-strong focus-visible:border-accent focus-visible:shadow-[0_0_0_1px_var(--accent)] focus-visible:outline-none disabled:opacity-40 aria-invalid:border-danger aria-invalid:focus-visible:shadow-[0_0_0_1px_var(--danger)]",
        className,
      )}
      {...props}
    />
